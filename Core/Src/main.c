...
uint8_t I2C_is_init(I2C_HandleTypeDef *i2c_handle)
{
  return i2c_handle->Mode != HAL_I2C_MODE_NONE;
}
...
{
  uint8_t data[] = {(value & 0xF00) >> 8, (value & 0xFF)};
  return HAL_I2C_Master_Transmit(i2c_handle, address << 1, data, 2, 1000);
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  if(mcp4725_init(&hi2c1, 0x60) != HAL_OK) {
    printf("MCP4725 DAC init failed\r\n");
  }
  else
  {
    printf("MCP4725 DAC initialized\r\n");
  }
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    // send "Hello, world!" over USB CDC
    uint16_t adc_value = HAL_ADC_GetValue(&hadc1);
    HAL_ADC_Start(&hadc1);

    printf("%d\t%d\r\n", dac_value, adc_value);

    // set dac_value to a random value between 0 and 4095
    dac_value += 64;
    if(dac_value > 4095) {
      dac_value = 0;
    }
